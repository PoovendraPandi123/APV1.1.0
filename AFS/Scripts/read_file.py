...
month_list = ["January", "Jan", "JAN", "February", "Feb", "FEB", "March", "Mar", "MAR", "April", "Apr", "APR", "May",
              "May", "MAY", "June", "Jun", "JUN", "July",
              "Jul", "JUL", "August", "Aug", "AUG", "September", "Sep", "SEP", "October", "Oct", "OCT", "November",
...
def convert_format(date_string):
    try:
        ...
        elif re.search("/", date_input) and (re.search("AM", date_input) or re.search("PM", date_input)) and re.search(
                "yes", date_key_word_var.lower()):
            date_input_proper = date_input.replace(" AM", "").replace(" PM", "")
            time_and_second = date_input_proper.split(" ")[1]
            date_value = date_input_proper.split(" ")[0]
            year_hiffen_time = date_value.split("/")[2]
...
def get_data_from_file(file_path, sheet_name, source_extension, attribute_list, column_start_row, password_protected,
    ...
    try:
        ...
        if source_extension in ["csv"]:
            data = pd.read_csv(file_path, skiprows=int(column_start_row) - 1, usecols=attribute_list,
                               converters=data_column_converter)[attribute_list]
        elif source_extension in ["xlsx", "xls"]:
            data = pd.read_excel(file_path, skiprows=int(column_start_row) - 1, usecols=attribute_list,
                                 converters=data_column_converter)[attribute_list]
        elif source_extension in ["xlsb"]:
            data = pd.read_excel(file_path, skiprows=int(column_start_row) - 1, usecols=attribute_list, engine="pyxlsb",
                                 converters=data_column_converter)[attribute_list]
        if len(data) > 0:
            ...
            data_filter = data_proper
            for i in range(0, len(data_proper.columns)):
                proper_index = i
                data_filter[data_filter.columns[proper_index]] = data_filter[
                    data_filter.columns[proper_index]].str.lstrip()
                data_filter[data_filter.columns[proper_index]] = data_filter[
                    data_filter.columns[proper_index]].str.rstrip()
                data_filter[data_filter.columns[proper_index]] = data_filter[
                    data_filter.columns[proper_index]].str.replace('\t', '')
                data_filter[data_filter.columns[proper_index]] = data_filter[
                    data_filter.columns[proper_index]].str.replace('\n', '')
                data_filter[data_filter.columns[proper_index]] = data_filter[
                    data_filter.columns[proper_index]].str.replace("'", "/#/")
                # data_filter[data_filter.columns[proper_index]] = data_filter[
                # data_filter.columns[proper_index]].str.replace("0", "/##/")
                # print("DataFilter")
